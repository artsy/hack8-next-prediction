...
export const LotsList: React.FC<Props> = ({ lots, saleSlug }) => {
  ...
  return (
    <>
      <BorderBox flexDirection='row' justifyContent="space-between" borderRadius={0} borderX="none">
        <Text variant="mediumText">LOTS</Text>
        <Flex>
          <Link href="#" underlineBehavior="none" color={ upcomingActive ? "purple100" : "black60" } hoverColor="purple100" mx={0.5} onClick={() => { setUpcomingActive(true) }}>
            <Text variant="mediumText">UPCOMING</Text>
          </Link>
          <Link href="#" underlineBehavior="none" color={ !upcomingActive ? "purple100" : "black60" } hoverColor="purple100" mx={0.5} onClick={() => { setUpcomingActive(false) }}>
